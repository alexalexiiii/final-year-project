...
export function AttachmentSelector({ mailboxItem, onSelectAttachment, onAnalysisComplete }: AttachmentSelectorProps) {
  ...
  const validateAttachment = (filename: string, size: number) => {
    const MAX_SIZE = 25 * 1024 * 1024; // 25MB

    if (size > MAX_SIZE) {
      return { valid: false, error: 'Attachment is too large (max 25MB)' };
    }

    const blockedExtensions = ['.exe', '.bat', '.ps1', '.js', '.vbs', '.scr'];

    const ext = filename.substring(filename.lastIndexOf('.')).toLowerCase();

    if (blockedExtensions.includes(ext)) {
      return { valid: false, error: 'Executable files are not allowed for analysis' };
    }
